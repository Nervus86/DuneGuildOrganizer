...
<template>
  <CardBox>
    <BaseLevel v-if="trend" class="mb-3" mobile>
      ...
      <div>
        <BaseButton :icon="mdiPlus" icon-w="w-4" icon-h="h-4" color="lightDark" small  @click="$emit('add-click')" />
        <BaseButton :icon="mdiCog" icon-w="w-4" icon-h="h-4" color="lightDark" small @click="$emit('settings-click')"/>
      </div>
    </BaseLevel>
    <BaseLevel mobile>
      <div>
